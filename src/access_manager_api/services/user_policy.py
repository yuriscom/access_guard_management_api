...
class IAMUserPolicyService:
    ...
    def get_policy(self, policy_id: int) -> Optional[IAMUserPolicy]:
    ...
    def get_policies_by_user(self, user_id: int) -> List[IAMUserPolicy]:
        return self.db.query(IAMUserPolicy).filter(IAMUserPolicy.user_id == user_id).all()

    def update_policy(self, policy_id: int, policy: IAMUserPolicyCreate) -> Optional[IAMUserPolicy]:
